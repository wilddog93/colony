...
const SidebarBody = ({ sidebarOpen, setSidebarOpen, children }: Props) => {
  ...
  const initiaLocalStorage: any = {
    sidebar: getFromLocalStorage("sidebar-body")
      ? JSON.parse(getFromLocalStorage("sidebar-body") || "{}")
      : [],
  };
  ...
  return (
    <Fragment>
      <aside
        className={`w-full max-w-xs lg:max-w-md hidden lg:flex flex-col overflow-y-hidden duration-300 ease-in-out ${
          sidebarOpen ? "" : "lg:w-0"
        }`}>
        {/* <!-- SIDEBAR HEADER --> */}
