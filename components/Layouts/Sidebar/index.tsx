...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: Props) => {
    ...
    return (
        <aside
            ...
            className={`absolute left-0 top-0 z-9999 flex h-screen w-full lg:w-80 flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
                }`}
        >
            {/* <!-- SIDEBAR HEADER --> */}
            <div className='flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5'>
                <Link href='/'>
                    <Image loader={myLoader} src="image/logo/logo.svg" alt='Logo' width={200} height={200} />
                </Link>
                                ...
                                {(handleClick: any, open: boolean) => {
                                    return (
                                        <React.Fragment>
                                            <button
                                                type='button'
                                                className={`w-full group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${(pathname === '/' ||
                                                    pathname.includes('building-management')) &&
