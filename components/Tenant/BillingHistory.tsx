...
const BillingHistory = () => {
  ...
  const [loading, setLoading] = useState(true);
      ...
      const columns = useMemo<ColumnDef<BillingProps, any>[]>(() => [
        {
            ...
            header: (info) => (
                <div className='uppercase text-left'>Payment Purpose</div>
            ),
            cell: ({ getValue, row }) => {
        ...
        },
        {
          ...
          header: (info) => (
              <div className='uppercase text-left'>Total Payment</div>
          ),
          cell: ({ getValue, row }) => {
      ...
      },
      {
        accessorKey: 'totalPaidBill',
        header: (info) => (
            <div className='uppercase text-left'>Due Date</div>
        ),
        cell: ({ getValue, row }) => {
    ...
    },
    {
      accessorKey: 'totalPaidBill',
      header: (info) => (
          <div className='uppercase text-left'>Payment Date</div>
      ),
      cell: ({ getValue, row }) => {
          let end = row?.original?.periodEnd
          return (
