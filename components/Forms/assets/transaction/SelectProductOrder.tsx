  ...
  const OrderComponent = (props: any) => {
    ...
    return (
      <div className="w-full divide-y-2 divide-gray border-2 border-gray rounded-lg text-xs">
        ...
        <div className="w-full grid col-span-1 grid-cols-3 divide-x-2 divide-gray">
          <div className="w-full p-4 tracking-wide">
            <div className="font-semibold text-sm">Vendor :</div>
            <p>{item?.vendor?.vendorName}</p>
          </div>
          ...
          <div className="w-full p-4 tracking-wide">
            ...
            <p>
              ...
              item?.orderDescription?.length > 70 &&
              !selected.includes(item?.id)
                ? `${item?.orderDescription.substring(70, 0)} ...`
                : item?.orderDescription || "-"}
            </p>
            <button
              onClick={() => onSelected(item?.id)}
              className={`text-primary focus:outline-none font-bold mt-2 underline w-full max-w-max ${
                item?.orderDescription?.length > 70 ? "" : "hidden"
              }`}>
              {selected.includes(item?.id) ? "Hide" : "Show"}
            </button>
          </div>
          ...
          <div className="w-full flex gap-1">
            <div className="w-[85%]">
              <DropdownSelect
                ...
                isDisabled={false}
                isMulti={false}
                placeholder="Choose purchase order no."
        ...
        <div className="w-full max-h-[250px] flex flex-col gap-2 overflow-x-hidden overflow-y-auto">
          {productSelected?.length > 0 ? (
            productSelected?.map((item: any, idx: any) => {
              return <OrderComponent key={idx} item={item} />;
            })
          ) : (
            <div className="text-sm italic text-gray-500 font-semibold">
              There is no purchase order data
            </div>
          )}
        </div>
      </div>
