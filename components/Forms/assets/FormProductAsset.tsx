...
export default function FormProductAsset({
  closeModal,
  items,
  setItems,
  token,
  ...
  const onAddProduct = (asset: any) => {
    if (!asset) return;
    let dataProducts =
      !productSelected || productSelected?.length == 0
        ? [asset]
        : [...productSelected, asset];
    let filterOpt = selectedOption?.filter((e) => e?.value != asset?.id);
  ...
  console.log(productSelected, "selected");
      ...
      <div className="w-full flex gap-2 border-2 border-gray rounded-lg px-4 py-2 text-xs">
        <div className="w-full flex items-center gap-1">
          <img
            src={
              assets?.product?.productImage
                ? `${url}product/productImage/${assets?.product?.productImage}`
                : defaultImage
            }
            alt=""
          ...
          <div className="w-full flex items-center justify-between">
            <span>{assets?.product?.productName}</span>
            <span>{assets?.id}</span>
          </div>
        </div>
        <div className="ml-auto">
          <button
            type="button"
            onClick={() => onDeleteHandler(assets)}
  ...
  const FormatOptionLabel = (props: any) => {
    return (
      <div className="w-full flex items-center justify-between">
        <div>{props?.product?.pruductName || props?.label}</div>
        <div>{props?.id}</div>
      </div>
    );
  };
      ...
      <ModalHeader
        ...
        <Fragment>
          <div className={`w-full flex flex-col gap-1 px-2`}>
            <h3 className="text-lg font-semibold">Add Product Asset</h3>
            <p className="text-gray-5 text-sm">
              Fill your product asset information.
            </p>
          </div>
        </Fragment>
      </ModalHeader>
          ...
          <div className="w-full max-h-[250px] flex flex-col gap-2 overflow-x-hidden overflow-y-auto">
            {productSelected?.length > 0 ? (
              productSelected?.map((item: any, idx: any) => {
                return <AssetComponent key={idx} assets={item} />;
              })
            ) : (
              <div className="px-4 text-xs text-gray-5">
                There is no product asset added
              </div>
            )}
          </div>
        </div>
