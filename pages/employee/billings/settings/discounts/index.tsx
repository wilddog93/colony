...
const BillingDiscount = ({ pageProps }: Props) => {
  moment.locale("id");
  ...
  // billing
  const { billingDiscounts, pending } = useAppSelector(
    selectBillingDiscountManagement
  );
  ...
  // data-table
  const [dataTable, setDataTable] = useState<BillingDiscountProps[]>([]);
  ...
  const [formData, setFormData] = useState<BillingDiscountProps | any>(null);
  ...
  const onOpenModalEdit = (items: any) => {
    let newData: BillingDiscountProps | any = {
      ...items,
      billingDiscountType: items?.billingDiscountType
        ? {
            value: items?.billingDiscountType,
            label: items?.billingDiscountType,
          }
        : null,
    };
    setFormData(newData);
  ...
  const columns = useMemo<ColumnDef<BillingDiscountProps, any>[]>(
    () => [
      {
        accessorKey: "billingDiscountId",
        header: (info) => <div className="uppercase">Discount No.</div>,
        cell: ({ row, getValue }) => {
          let value = getValue() || "-";
          return (
      ...
      {
        accessorKey: "billingDiscountName",
      ...
      {
        accessorKey: "billingDiscountType",
      ...
      {
        accessorKey: "billingDiscountTotal",
        header: (info) => (
          <div className="uppercase w-full text-center">Total</div>
        ),
        cell: ({ row, getValue }) => {
          const value = getValue();
          const { billingDiscountType } = row?.original;
          return (
  ...
  const filters = useMemo(() => {
    ...
    const search = {
      $and: [
        {
          $or: [
            { billingDiscountName: { $contL: query?.search } },
            { billingDiscountId: { $contL: query?.search } },
            { billingDiscountType: { $contL: query?.search } },
          ],
        },
      ],
    };
  ...
  useEffect(() => {
    if (token)
      dispatch(getBillingDiscount({ token, params: filters.queryObject }));
  }, [token, filters]);

  console.log(billingDiscounts, "billingDiscounts");

  useEffect(() => {
    let newArr: any[] = [];
    const { data, pageCount, total } = billingDiscounts;
  ...
  const onDelete = (value: any) => {
    ...
    dispatch(
      deleteBillingDiscount({
        ...
        id: value?.id,
        isSuccess() {
          toast.dark("Discount has been deleted");
          dispatch(getBillingDiscount({ token, params: filters.queryObject }));
          onCloseModalDelete();
        },
        isError() {
            ...
            <div className="w-full flex flex-col lg:flex-row items-start lg:items-center justify-between gap-2">
              ...
              <div className="w-full lg:max-w-max flex items-center justify-center gap-2 lg:ml-auto">
                <Button
                  ...
                  variant="primary">
                  <span className="hidden lg:inline-block">New Discount</span>
      ...
      <Modal size="small" onClose={onCloseModalAdd} isOpen={isOpenAdd}>
        <Fragment>
          <ModalHeader
            ...
            <div className="flex flex-col gap-1">
              <h3 className="text-base font-semibold">New Discount</h3>
              <p className="text-gray-5 text-sm">
                Fill your information discount data
              </p>
            </div>
          </ModalHeader>
          ...
          <DiscountForm
            ...
            getData={() =>
              dispatch(
                getBillingDiscount({ token, params: filters.queryObject })
              )
            }
          />
        </Fragment>
