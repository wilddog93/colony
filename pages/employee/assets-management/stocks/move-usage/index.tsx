...
const RequestMoveUsage = ({ pageProps }: Props) => {
  moment.locale("id");
        ...
        cell: ({ row, getValue }) => {
          ...
          return (
            <button
              onClick={() =>
                router.push({
                  pathname: `/employee/assets-management/stocks/move-usage/${id}`,
                  query: {
  ...
  const goToForm = (item: any) => {
    if (!item?.value) {
      ...
      return router.push({
        pathname: `/employee/assets-management/stocks/move-usage/form`,
        query: {
          type: item?.value,
        },
      });
...
export default RequestMoveUsage;
