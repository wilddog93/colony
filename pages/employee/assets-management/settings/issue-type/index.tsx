...
const IssueType = ({ pageProps }: Props) => {
  ...
  const { issueTypes, pending } = useAppSelector(selectIssueType);
  ...
  const goToTask = (id: any) => {
    if (!id) return;
    return router.push({
      pathname: `/employee/tasks/settings/issue-type/${id}`,
    });
  };
  ...
  const filters = useMemo(() => {
    ...
    const search = {
      $and: [
        {
          $or: [
            { issueTypeName: { $contL: query?.search } },
            { issueTypeDescription: { $contL: query?.search } },
            { issueTypeCode: { $contL: query?.search } },
          ],
        },
      ],
    };
  ...
  useEffect(() => {
    if (token) dispatch(getIssueTypes({ token, params: filters.queryObject }));
  }, [token, filters]);
    ...
    dispatch(
      deleteIssueTypes({
        ...
        isSuccess() {
          ...
          dispatch(getIssueTypes({ token, params: filters.queryObject }));
          onCloseModalDelete();
        },
        isError() {
