...
const IssueCategory = ({ pageProps }: Props) => {
  ...
  const { taskCategories, pending } = useAppSelector(selectTaskCategory);
  ...
  const goToTask = (id: any) => {
    if (!id) return;
    return router.push({
      pathname: `/employee/tasks/settings/task-category/${id}`,
    });
  };

  // color
  const genColorData = (value: any) => {
    // #333A48
    let color = value;
    if (!value) return "-";
    return color;
  ...
  const columns = useMemo<ColumnDef<PropsData, any>[]>(
    () => [
      ...
      {
        accessorKey: "taskCategoryFillColor",
        cell: ({ row, getValue }) => {
          let color = getValue();
          return (
            <div className="flex items-center gap-2">
              <span
                className={`w-5 h-5 rounded-lg ${
                  color ? "shadow-card border-gray" : ""
                }`}
                style={{
                  backgroundColor: !color ? "#FFFFFF" : genColorData(color),
                }}></span>
              <p>{genColorData(color)}</p>
            </div>
          );
        },
        header: (props) => (
          <div className="w-full text-left uppercase">Fill Color</div>
        ),
        footer: (props) => props.column.id,
        enableColumnFilter: false,
        size: 350,
      ...
      {
        accessorKey: "taskCategoryTextColor",
        cell: ({ row, getValue }) => {
          let color = getValue();
          return (
            <div className="flex items-center gap-2">
        ...
        header: (props) => (
          <div className="w-full text-left uppercase">Text Color</div>
        ),
        footer: (props) => props.column.id,
        enableColumnFilter: false,
        size: 350,
  ...
  const filters = useMemo(() => {
    ...
    const search = {
      $and: [
        {
          $or: [
            { taskCategoryName: { $contL: query?.search } },
            { taskCategoryDescription: { $contL: query?.search } },
            { taskCategoryFillColor: { $contL: query?.search } },
            { taskCategoryTextColor: { $contL: query?.search } },
          ],
        },
      ],
    };
  ...
  useEffect(() => {
    if (token)
      dispatch(getTaskCategories({ token, params: filters.queryObject }));
  }, [token, filters]);

  useEffect(() => {
    let newArr: any[] = [];
    const { data, pageCount, total } = taskCategories;
  ...
  const onDelete = (value: any) => {
    ...
    dispatch(
      deleteTaskCategory({
        ...
        id: value?.id,
        isSuccess() {
          toast.dark("Task category has been deleted");
          dispatch(getTaskCategories({ token, params: filters.queryObject }));
          onCloseModalDelete();
        },
        isError() {
        ...
        <Fragment>
          <ModalHeader
            ...
            <div className="w-full flex flex-col gap-1">
              <h3 className="text-base font-semibold">Delete Task Category</h3>
              <p className="text-xs text-gray-5">
                Are you sure to delete task category ?
              </p>
            </div>
          </ModalHeader>
