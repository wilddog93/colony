    ...
    return (
        ...
        >
            <div className='relative overflow-hidden w-full h-full flex items-center rounded-xl bg-white shadow-default p-10'>
                ...
                <div className={`relative hidden w-full lg:w-1/2 h-full lg:block transition-transform duration-300 ease-in-out border bg-primary text-white border-stroke rounded-3xl translate-x-0 ${!isSignUp ? "translate-x-full" : ""}`}>

                    <div className="w-full h-full flex flex-col items-center justify-between">
                        <Link className={`w-full pt-5.5 flex items-center gap-4 px-10 ${!isSignUp ? "hidden" : ""}`} href='/'>
                            <img src="../image/logo/logo-icon-white.png" alt="logo" />
                            <h2 className='text-lg text-white sm:text-title-lg'>
                                Colony.
                            </h2>
                        </Link>
                        <div className={`w-full flex flex-col px-10 justify-center ${!isSignUp ? "h-full" : ""}`}>
                    ...
                    <div className={`absolute z-40 inset-y-1/2 ${!isSignUp ? "right-0" : ""}`}>
                        <Tooltip
                            className={`tooltip w-full text-sm bg-[#111F2C3D] p-2 rounded-lg focus:outline-none ${!isSignUp ? "rounded-tr-none rounded-br-none" : "rounded-tl-none rounded-bl-none"}`}
                            classTooltip='p-5 rounded-xl shadow-lg z-1 font-bold w-full min-w-max'
                            tooltip={isSignUp ? "Go to Sign in" : "Go to Sign up"}
                            color='light'
                            position={isSignUp ? "right" : "left"}
                        >
                            <MdArrowBack onClick={() => handleChangePage({ callback: () => setForm({}) })} className={`w-10 h-10 transition-transform ease-in-out duration-1000 ${!isSignUp ? "" : "rotate-180"}`} />
                        </Tooltip>
                    </div>
                </div>
