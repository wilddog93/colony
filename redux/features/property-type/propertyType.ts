...
export const getPropertyType = createAsyncThunk<
  ...
  DefaultGetData,
  { state: RootState }
>("property-type", async (params, { getState }) => {
  ...
  try {
    const response = await axios.get("api/propertyType", config);
...
>("property-type/id", async (params, { getState }) => {
  ...
  try {
    const response = await axios.get(`api/propertyType/${params.id}`, config);
    const { data, status } = response;
    if (status == 200) {
      return data;
    } else {
...
export const createPropertyType = createAsyncThunk<
  ...
  PropertyTypeData,
  { state: RootState }
>("create/property-type", async (params, { getState }) => {
  let config: HeadersConfiguration = {
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
  ...
  try {
    const response = await axios.post("api/propertyType", params.data, config);
    const { data, status } = response;
    if (status == 201) {
      return data;
    } else {
...
export const updatePropertyType = createAsyncThunk<
  ...
  PropertyTypeData,
  { state: RootState }
>("update/property-type", async (params, { getState }) => {
  let config: HeadersConfiguration = {
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
