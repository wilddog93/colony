...
export const getDomainId = createAsyncThunk<
  any,
  DefaultGetData,
  { state: RootState }
>("/domain/{id}", async (params, { getState }) => {
  let config: HeadersConfiguration = {
  ...
  try {
    const response = await axios.get(`api/domain/${params.id}`, config);
    const { data, status } = response;
    if (status == 200) {
      return data;
    } else {
...
export const getAccessDomain = createAsyncThunk<
  any,
  DefaultGetData,
  { state: RootState }
...
export const createAccessDomain = createAsyncThunk<
  any,
  DomainData,
  { state: RootState }
>("post/web/domain", async (params, { getState }) => {
  let config: HeadersConfiguration = {
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
  ...
  try {
    const response = await axios.post("api/domain", params.data, config);
    const { data, status } = response;
    if (status == 201) {
      params.isSuccess();
      return data;
...
// update domain
export const updateAccessDomain = createAsyncThunk<
  any,
  DomainData,
  { state: RootState }
>("patch/web/domain", async (params, { getState }) => {
  let config: HeadersConfiguration = {
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
  ...
  try {
    const response = await axios.patch(`api/domain`, params.data, config);
    const { data, status } = response;
    if (status == 200) {
      params.isSuccess();
      return data;
