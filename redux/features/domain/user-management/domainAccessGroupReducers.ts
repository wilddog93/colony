...
export const createDomainAccessGroup = createAsyncThunk<any, DomainData, { state: RootState }>('/post/domainAccessGroup', async (params, { getState }) => {
    let config: HeadersConfiguration = {
        headers: {
            "Content-Type": "application/json",
            "Accept": "application/json",
    ...
    try {
        const response = await axios.post("domainAccessGroup", params.data, config);
        const { data, status } = response;
        if (status == 201) {
            params.isSuccess()
            return data
